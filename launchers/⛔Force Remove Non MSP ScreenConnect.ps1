...
<#
.SYNOPSIS
    Slim Level.io Launcher for Force Remove Non-MSP ScreenConnect

.DESCRIPTION
    Removes unauthorized ScreenConnect (ConnectWise Control) instances while
    preserving your MSP instance. Devices hosting the ScreenConnect server
...
.NOTES
    ...
    - {{policy_screenconnect_instance_id}}          : MSP's ScreenConnect instance ID to whitelist
    - {{policy_screenconnect_server}}               : Set to "true" if device hosts SC server
...
# Clean up ScreenConnect variables
