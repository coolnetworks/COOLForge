#!/bin/bash
# MeshCentral Policy Launcher - Linux
...
export POLICY_LINUX_INSTALL="{{cf_policy_meshcentral_linux_install}}"
...
# CONFIGURATION
# ============================================================
SCRIPT_NAME="👀meshcentral-linux.sh"
REPO_OWNER="coolnetworks"
REPO_NAME="COOLForge"
LINUX_CACHE_BASE="/var/cache"
...
if [ -n "$MSP_SCRATCH_FOLDER" ] && [[ "$MSP_SCRATCH_FOLDER" != "{{cf_"* ]]; then
    ...
    SCRATCH_FOLDER="${LINUX_CACHE_BASE}/${FOLDER_NAME}"
else
    SCRATCH_FOLDER="${LINUX_CACHE_BASE}/COOLForge"
fi
...
if [ -z "$POLICY_LINUX_INSTALL" ] || [[ "$POLICY_LINUX_INSTALL" == "{{cf_"* ]]; then
    MISSING_FIELDS="${MISSING_FIELDS}cf_policy_meshcentral_linux_install, "
fi
...
if [ -n "$MISSING_FIELDS" ]; then
    ...
    echo "Alert: MeshCentral Linux launcher cannot run - missing custom fields. Run the Windows version of this script first on any Windows device and it will create what is required. Then ensure you fill the custom fields: $MISSING_FIELDS"
    exit 1
fi
...
ENCODED_SCRIPT_NAME="%F0%9F%91%80meshcentral-linux.sh"
...
# ============================================================
echo "[*] MeshCentral Linux Launcher v2026.01.19.03"
