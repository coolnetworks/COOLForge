...
$script:AlertQueue = @()            # Queued technician alerts to send on script completion
$script:AlertApiKey = $null         # API key for sending alerts (set via Initialize-LevelScript)
...
.PARAMETER ApiKey
    Level.io API key for sending technician alerts. Typically "{{cf_apikey}}".
    When provided, enables automatic alert queue flushing on script completion.
...
function Invoke-LevelScript {
    ...
    try {
        ...
        # Flush any queued technician alerts
        if ($script:AlertQueue.Count -gt 0) {
            Send-TechnicianAlertQueue | Out-Null
        }
        ...
        # Still flush alerts on failure - technicians need to know
        if ($script:AlertQueue.Count -gt 0) {
            Send-TechnicianAlertQueue | Out-Null
        }
...
function Get-EmojiMap {
    ...
    return @{
        ...
        "🧑‍💻" = "Technician"  # U+1F9D1 U+200D U+1F4BB - Technician workstation for alerts
...
<#
.SYNOPSIS
    Sends an alert to technician workstations via Level.io custom field.

.DESCRIPTION
    Creates an alert entry that technician workstations will pick up and display
    as a Windows toast notification. Alerts are stored in a custom field and
    include metadata for routing, priority, and acknowledgment.

    Alert Flow:
    1. Script calls Send-TechnicianAlert with message details
    2. Alert is written to cf_technician_alerts (JSON array)
...
.PARAMETER Title
    Short alert title (displayed as notification header).
    Example: "Manual Install Required"

.PARAMETER Message
    Detailed alert message explaining what action is needed.
    Example: "Huntress installer failed on ACME-PC01, needs manual intervention"

.PARAMETER ClientName
    Name of the client/organization. Used for routing and display.
    Typically extracted from Level group path.

.PARAMETER DeviceHostname
    Hostname of the device that triggered the alert.
    Typically "{{level_device_hostname}}".

.PARAMETER Priority
    Alert priority level. Default: "Normal"
...
.PARAMETER TechnicianName
    Optional - route alert to a specific technician.
    If not specified, alert goes to all technicians.

.PARAMETER ExpiresInMinutes
    How long the alert remains active. Default: 1440 (24 hours)
...
.OUTPUTS
    Hashtable with:
    - Success: $true if alert was sent, $false on failure
    - AlertId: Unique ID of the created alert
    - Error: Error message if failed

.EXAMPLE
    # Send a basic alert
    $Result = Send-TechnicianAlert -ApiKey $ApiKey `
...
.EXAMPLE
    # Send a high-priority alert to a specific technician
    Send-TechnicianAlert -ApiKey $ApiKey `
...
.NOTES
    Requires cf_technician_alerts custom field to exist in Level.io.
    Alerts are stored as a JSON array in this field.
#>
function Send-TechnicianAlert {
    ...
    Write-LevelLog "Sending technician alert: $Title" -Level "INFO"
    Write-LevelLog "  Device: $DeviceHostname | Client: $ClientName | Priority: $Priority" -Level "DEBUG"

    # Find the alerts custom field
    $FieldsResult = Invoke-LevelApiCall -Uri "$BaseUrl/custom_fields" -ApiKey $ApiKey -Method "GET"
    if (-not $FieldsResult.Success) {
        Write-LevelLog "Failed to get custom fields: $($FieldsResult.Error)" -Level "ERROR"
        return @{ Success = $false; AlertId = $null; Error = $FieldsResult.Error }
    }

    $AlertsField = $FieldsResult.Data.data | Where-Object { $_.name -eq "cf_technician_alerts" } | Select-Object -First 1
    if (-not $AlertsField) {
        Write-LevelLog "cf_technician_alerts custom field not found - please create it in Level.io" -Level "ERROR"
        return @{ Success = $false; AlertId = $null; Error = "Custom field not found" }
    }

    # Find a central location to store alerts (could be a specific group or use device-level)
    # For now, we'll store alerts on the source device's group
    $Device = Find-LevelDevice -ApiKey $ApiKey -Hostname $DeviceHostname -BaseUrl $BaseUrl
    if (-not $Device) {
        Write-LevelLog "Could not find device in Level.io" -Level "WARN"
        return @{ Success = $false; AlertId = $null; Error = "Device not found" }
    }

    # Get current alerts from the group
    $GroupResult = Invoke-LevelApiCall -Uri "$BaseUrl/groups/$($Device.group_id)" -ApiKey $ApiKey -Method "GET"
    $CurrentAlerts = @()

    if ($GroupResult.Success -and $GroupResult.Data.custom_field_values) {
        ...
        if ($AlertsValue -and $AlertsValue.value) {
            try {
                $CurrentAlerts = @($AlertsValue.value | ConvertFrom-Json)
            }
            catch {
                $CurrentAlerts = @()
            }
    ...
    $Now = Get-Date
    $CurrentAlerts = @($CurrentAlerts | Where-Object {
        try { [datetime]$_.expires -gt $Now } catch { $false }
    })
    ...
    $UpdateBody = @{
        value = ($CurrentAlerts | ConvertTo-Json -Compress)
    }

    $UpdateResult = Invoke-LevelApiCall `
        -Uri "$BaseUrl/groups/$($Device.group_id)/custom_field_values/$($AlertsField.id)" `
        -ApiKey $ApiKey `
        -Method "PATCH" `
        -Body $UpdateBody

    if ($UpdateResult.Success) {
        Write-LevelLog "Alert sent successfully (ID: $AlertId)" -Level "SUCCESS"
        return @{ Success = $true; AlertId = $AlertId; Error = $null }
    }
    else {
        Write-LevelLog "Failed to send alert: $($UpdateResult.Error)" -Level "ERROR"
        return @{ Success = $false; AlertId = $null; Error = $UpdateResult.Error }
    }
}

<#
.SYNOPSIS
    Queues a technician alert to be sent when the script completes.

.DESCRIPTION
    Adds an alert to the module's internal queue. Queued alerts are automatically
    sent when Invoke-LevelScript completes (success or failure), or can be
    manually flushed with Send-TechnicianAlertQueue.

    This is the recommended way to send alerts - it batches multiple alerts
    into a single API call and ensures alerts are sent even if the script
    encounters errors.

    Requires Initialize-LevelScript to be called with -ApiKey parameter.

.PARAMETER Title
    Short title for the alert (shown in toast notification header).

.PARAMETER Message
    Detailed message body for the alert.
...
.PARAMETER TechnicianName
    Optional technician name for routing. If specified, only workstations
    with matching technician tag will show the alert.

.PARAMETER ExpiresInMinutes
    How long before alert expires. Default: 1440 (24 hours).

.OUTPUTS
    Hashtable with Success and QueueLength properties.
...
function Add-TechnicianAlert {
    ...
    if (-not $script:Initialized) {
        Write-LevelLog "Add-TechnicianAlert called before Initialize-LevelScript" -Level "WARN"
        return @{ Success = $false; QueueLength = 0; Error = "Not initialized" }
    }
    ...
    # Add to queue
    $script:AlertQueue += $Alert

    Write-LevelLog "Alert queued: $Title (ID: $AlertId, Queue: $($script:AlertQueue.Count))" -Level "DEBUG"

    return @{ Success = $true; QueueLength = $script:AlertQueue.Count; AlertId = $AlertId }
}

<#
.SYNOPSIS
    Sends all queued technician alerts to Level.io.

.DESCRIPTION
    Sends the alert queue by pushing all pending alerts to the
    cf_technician_alerts custom field. Called automatically by
    Invoke-LevelScript on completion.

    Alerts are batched by group and sent in a single API call per group.
    After successful send, the queue is cleared.

    Requires Initialize-LevelScript to be called with -ApiKey parameter.

.PARAMETER Force
    Send alerts even if ApiKey wasn't provided to Initialize-LevelScript.
    Requires -ApiKey parameter when using -Force.

.PARAMETER ApiKey
    Override API key. Only needed with -Force or if ApiKey wasn't
    provided to Initialize-LevelScript.

.OUTPUTS
    Hashtable with Success, AlertsSent, and Errors properties.

.EXAMPLE
    # Automatic send (called by Invoke-LevelScript)
    Send-TechnicianAlertQueue

.EXAMPLE
    # Manual send with override key
    Send-TechnicianAlertQueue -ApiKey "your-api-key" -Force
#>
function Send-TechnicianAlertQueue {
    ...
    # Use provided ApiKey or fall back to stored one
    $EffectiveApiKey = if ($ApiKey) { $ApiKey } else { $script:AlertApiKey }

    if (-not $EffectiveApiKey) {
        if ($script:AlertQueue.Count -gt 0) {
            Write-LevelLog "Alert queue has $($script:AlertQueue.Count) alerts but no ApiKey - alerts not sent" -Level "WARN"
        }
        return @{ Success = $false; AlertsSent = 0; Error = "No ApiKey configured" }
    }

    if ($script:AlertQueue.Count -eq 0) {
        return @{ Success = $true; AlertsSent = 0; Error = $null }
    }

    Write-LevelLog "Flushing $($script:AlertQueue.Count) queued alert(s)..." -Level "INFO"

    # Find the alerts custom field
    $FieldsResult = Invoke-LevelApiCall -Uri "$BaseUrl/custom_fields" -ApiKey $EffectiveApiKey -Method "GET"
    if (-not $FieldsResult.Success) {
        Write-LevelLog "Failed to get custom fields: $($FieldsResult.Error)" -Level "ERROR"
        return @{ Success = $false; AlertsSent = 0; Error = $FieldsResult.Error }
    }

    $AlertsField = $FieldsResult.Data.data | Where-Object { $_.name -eq "cf_technician_alerts" } | Select-Object -First 1
    if (-not $AlertsField) {
        Write-LevelLog "cf_technician_alerts custom field not found" -Level "ERROR"
        return @{ Success = $false; AlertsSent = 0; Error = "Custom field not found" }
    }

    # Find the device to get group ID
    $Device = Find-LevelDevice -ApiKey $EffectiveApiKey -Hostname $script:DeviceHostname -BaseUrl $BaseUrl
    if (-not $Device) {
        Write-LevelLog "Could not find device in Level.io" -Level "WARN"
        return @{ Success = $false; AlertsSent = 0; Error = "Device not found" }
    }

    # Get current alerts from the group
    $GroupResult = Invoke-LevelApiCall -Uri "$BaseUrl/groups/$($Device.group_id)" -ApiKey $EffectiveApiKey -Method "GET"
    $CurrentAlerts = @()
    ...
    # Add all queued alerts
    $CurrentAlerts += $script:AlertQueue
    ...
    if ($UpdateResult.Success) {
        $SentCount = $script:AlertQueue.Count
        $script:AlertQueue = @()
        Write-LevelLog "Flushed $SentCount alert(s) successfully" -Level "SUCCESS"
        return @{ Success = $true; AlertsSent = $SentCount; Error = $null }
    }
    else {
        Write-LevelLog "Failed to flush alerts: $($UpdateResult.Error)" -Level "ERROR"
        return @{ Success = $false; AlertsSent = 0; Error = $UpdateResult.Error }
    }
}

<#
.SYNOPSIS
    Checks if the current device is a technician workstation.

.DESCRIPTION
    Checks device tags for the technician tag (U+1F9D1 U+200D U+1F4BB technician)
    to determine if this device should receive technician alerts.

    The tag format is: {emoji}technician (e.g., "technician" with the
    technician emoji prefix). This is detected using Get-EmojiMap.

.PARAMETER DeviceTags
    Comma-separated list of device tags from Level.io.
    Typically "{{level_tag_names}}".

.OUTPUTS
    $true if this is a technician workstation, $false otherwise.
...
.EXAMPLE
    # Check tags and get technician name
    $IsTech = Test-TechnicianWorkstation -DeviceTags $DeviceTags
...
function Test-TechnicianWorkstation {
    ...
    if ([string]::IsNullOrWhiteSpace($DeviceTags)) {
        return $false
    }
    ...
    # Get emoji map to find technician patterns
    $EmojiMap = Get-EmojiMap

    # Parse tags
    $TagArray = $DeviceTags -split "," | ForEach-Object { $_.Trim() } | Where-Object { $_ }

    # Look for technician tag
    foreach ($Tag in $TagArray) {
        foreach ($Emoji in $EmojiMap.Keys) {
...
.DESCRIPTION
    Parses device tags to find the technician tag and extracts the name
    portion after the emoji prefix. For example, "technician" tag with
    name "John" would be "{emoji}John".
...
.OUTPUTS
    Technician name string, or empty string if not found.
...
function Get-TechnicianName {
    ...
    # Look for technician tag and extract name
    foreach ($Tag in $TagArray) {
        foreach ($Emoji in $EmojiMap.Keys) {
            if ($EmojiMap[$Emoji] -eq "Technician" -and $Tag.StartsWith($Emoji)) {
